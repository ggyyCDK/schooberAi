...
var ExecutionEngine = class extends chunkYT4RGZYO_cjs.MastraBase {
  mastra;
  constructor({ mastra }) {
    super({ name: "ExecutionEngine", component: chunkIDDUQR6P_cjs.RegisteredLogger.WORKFLOW });
    this.mastra = mastra;
  }
  ...
  async execute(params) {
    ...
    for (let i = startIdx; i < steps.length; i++) {
      ...
      try {
        ...
        const error = e instanceof chunk3BFAQTC3_cjs.MastraError ? e : new chunk3BFAQTC3_cjs.MastraError(
          {
...
function createStep(params) {
  ...
  if (params instanceof chunkA4TGKXTE_cjs.Agent) {
    return {
      ...
      execute: wrapExecute(async ({ inputData, [chunkNFXTYMWZ_cjs.EMITTER_SYMBOL]: emitter, runtimeContext, abortSignal, abort }) => {
        let streamPromise = {};
  ...
  if (params instanceof chunkH65GI7JY_cjs.Tool) {
...
var Workflow = class extends chunkYT4RGZYO_cjs.MastraBase {
  id;
  description;
  inputSchema;
  outputSchema;
  ...
  constructor({
    ...
    super({ name: id, component: chunkIDDUQR6P_cjs.RegisteredLogger.WORKFLOW });
    this.id = id;
   ...
   */
  sleep(duration) {
    const id = `sleep_${this.#mastra?.generateId() || crypto.randomUUID()}`;
  ...
  map(mappingConfig) {
    if (typeof mappingConfig === "function") {
      const mappingStep2 = createStep({
        id: `mapping_${this.#mastra?.generateId() || crypto.randomUUID()}`,
        inputSchema: zod.z.object({}),
        outputSchema: zod.z.object({}),
  ...
  createRun(options) {
    ...
    const runIdToUse = options?.runId || this.#mastra?.generateId() || crypto.randomUUID();
