...
var chunkMITKB4IU_cjs = require('./chunk-MITKB4IU.cjs');
var chunkVZUGB775_cjs = require('./chunk-VZUGB775.cjs');
  ...
  async moderateContent(content) {
    ...
    try {
      const response = await this.moderationAgent.generate(prompt, {
        output: z3__default.default.object({
          category_scores: z3__default.default.object(this.categories.reduce((props, category) => {
            props[category] = z3__default.default.number().min(0).max(1).optional();
            return props;
          }, {})).optional(),
          reason: z3__default.default.string().optional()
        }),
        temperature: 0
      });
  ...
  async detectPromptInjection(content) {
    ...
    try {
      const response = await this.detectionAgent.generate(prompt, {
        output: z3__default.default.object({
          categories: z3__default.default.object(this.detectionTypes.reduce((props, type) => {
            props[type] = z3__default.default.number().min(0).max(1).optional();
            return props;
          }, {})).optional(),
          reason: z3__default.default.string().optional(),
          rewritten_content: z3__default.default.string().optional()
        }),
        temperature: 0
      });
      const result = response.object;
...
var PIIDetector = class _PIIDetector {
    ...
    try {
      const response = await this.detectionAgent.generate(prompt, {
        output: z3__default.default.object({
          ...
          detections: z3__default.default.array(z3__default.default.object({
            type: z3__default.default.string(),
            value: z3__default.default.string(),
            confidence: z3__default.default.number().min(0).max(1),
            start: z3__default.default.number(),
            end: z3__default.default.number(),
            redacted_value: z3__default.default.string().optional()
          })).optional(),
          redacted_content: z3__default.default.string().optional()
        }),
        temperature: 0
      });
      const result = response.object;
   ...
   */
  hashValue(value) {
    return `[HASH:${crypto2__default.default.createHash("sha256").update(value).digest("hex").slice(0, 8)}]`;
  }
  /**
   * Extract text content from message for analysis
   */
...
var LanguageDetector = class _LanguageDetector {
  ...
  async detectLanguage(content) {
    ...
    try {
      const response = await this.detectionAgent.generate(prompt, {
        output: z3__default.default.object({
          iso_code: z3__default.default.string().optional(),
          confidence: z3__default.default.number().min(0).max(1).optional(),
          translated_text: z3__default.default.string().optional()
        }),
        temperature: 0
      });
...
_Agent_decorators = [chunkV6NA7QLK_cjs.InstrumentClass({
  prefix: "agent",
...
exports.Agent = class Agent extends (_a = chunkYT4RGZYO_cjs.MastraBase) {
  id;
  name;
  #instructions;
  #description;
  ...
  constructor(config) {
    super({
      component: chunkIDDUQR6P_cjs.RegisteredLogger.AGENT
    });
    ...
    if (!config.model) {
      const mastraError = new chunk3BFAQTC3_cjs.MastraError({
    ...
    if (config.voice) {
      ...
      this.#voice = new chunkVZUGB775_cjs.DefaultVoice();
    }
  ...
  async getVoice({
    ...
    if (this.#voice) {
      ...
      return new chunkVZUGB775_cjs.DefaultVoice();
    }
  }
  get instructions() {
  ...
  get tools() {
    ...
    return chunkMW267UVU_cjs.ensureToolProperties(this.#tools);
  }
  getTools({
    runtimeContext = new chunkGPWMM745_cjs.RuntimeContext()
  } = {}) {
    ...
    return resolveMaybePromise(result, tools => {
      ...
      return chunkMW267UVU_cjs.ensureToolProperties(tools);
    });
  }
  get llm() {
  ...
  getLLM({
    ...
    return resolveMaybePromise(modelToUse, resolvedModel => {
      const llm = new chunkMITKB4IU_cjs.MastraLLM({
        model: resolvedModel,
        mastra: this.#mastra
      });
      if (this.#primitives) {
  ...
  async generateTitleFromUserMessage({
    ...
    const normMessage = new chunkOFZZZ22B_cjs.MessageList().add(message, "user").get.all.ui().at(-1);
    if (!normMessage) {
  ...
  async genTitle(userMessage, runtimeContext, model, instructions) {
    try {
      if (userMessage) {
        const normMessage = new chunkOFZZZ22B_cjs.MessageList().add(userMessage, "user").get.all.ui().at(-1);
        if (normMessage) {
  ...
  async fetchMemory({
    ...
    userMessages,
    systemMessage,
    messageList = new chunkOFZZZ22B_cjs.MessageList({
      threadId,
      resourceId
    }),
    runtimeContext = new chunkGPWMM745_cjs.RuntimeContext()
...
`) ?? void 0;
      ...
      const returnList = new chunkOFZZZ22B_cjs.MessageList().addSystem(systemMessages).add(processedMemoryMessages, "memory").add(newMessages, "user");
      return {
        threadId: thread.id,
  ...
  async getAssignedTools({
    ...
    const assignedCoreToolEntries = await Promise.all(assignedToolEntries.map(async ([k, tool]) => {
      ...
      return [k, chunkMW267UVU_cjs.makeCoreTool(tool, options)];
    }));
    ...
    if (toolsFromToolsets.length > 0) {
      ...
      for (const toolset of toolsFromToolsets) {
        for (const [toolName, tool] of Object.entries(toolset)) {
          ...
          const convertedToCoreTool = chunkMW267UVU_cjs.makeCoreTool(toolObj, options, "toolset");
  ...
  async getClientTools({
    ...
    if (clientToolsForInput.length > 0) {
      ...
      for (const [toolName, tool] of clientToolsForInput) {
        ...
        const convertedToCoreTool = chunkMW267UVU_cjs.makeCoreTool(rest, options, "client-tool");
        toolsForRequest[toolName] = convertedToCoreTool;
      }
    }
    return toolsForRequest;
  ...
  async convertTools({
    ...
    if (this.#mastra) {
      mastraProxy = chunkMW267UVU_cjs.createMastraProxy({
        mastra: this.#mastra,
        logger
      });
    }
  ...
  __primitive({
    ...
    return {
      before: async () => {
        ...
        const messageList = new chunkOFZZZ22B_cjs.MessageList({
          threadId,
          resourceId,
        ...
        let [memoryMessages, memorySystemMessage] = existingThread ? await Promise.all([this.getMemoryMessages({
          ...
          vectorMessageSearch: new chunkOFZZZ22B_cjs.MessageList().add(messages, `user`).getLatestUserContent() || "",
          memoryConfig,
          runtimeContext
...
${(() => {
            let result = ``;
            const messages2 = new chunkOFZZZ22B_cjs.MessageList().add(resultsFromOtherThreads, "memory").get.all.v1();
            let lastYmd = null;
...
`) ?? void 0;
        ...
        const processedList = new chunkOFZZZ22B_cjs.MessageList({
          threadId: threadObject.id,
          resourceId,
          generateMessageId: this.#mastra?.generateId?.bind(this.#mastra),
          // @ts-ignore Flag for agent network messages
      ...
      after: async ({
        ...
        const messageListResponses = new chunkOFZZZ22B_cjs.MessageList({
          threadId,
          resourceId,
          generateMessageId: this.#mastra?.generateId?.bind(this.#mastra),
          // @ts-ignore Flag for agent network messages
        ...
        if (memory && resourceId && thread2) {
          try {
            ...
            if (e instanceof chunk3BFAQTC3_cjs.MastraError) {
              throw e;
            }
            const mastraError = new chunk3BFAQTC3_cjs.MastraError({
  ...
  async prepareLLMOptions(messages, options) {
    ...
    const activeSpan = chunkV6NA7QLK_cjs.Telemetry.getActiveSpan();
    ...
    if (Object.keys(baggageEntries).length > 0) {
      chunkV6NA7QLK_cjs.Telemetry.setBaggage(baggageEntries);
    }
    const memory = await this.getMemory({
      runtimeContext
    });
  ...
  async speak(input, options) {
    ...
    try {
      ...
      if (e instanceof chunk3BFAQTC3_cjs.MastraError) {
        ...
        err = new chunk3BFAQTC3_cjs.MastraError({
...
function agentToStep(agent, {
  ...
  return {
    ...
    inputSchema: z3.z.object({
      prompt: z3.z.string(),
      resourceId: z3.z.string().optional(),
      threadId: z3.z.string().optional()
    }),
...
var LegacyWorkflow = class extends chunkYT4RGZYO_cjs.MastraBase {
  name;
  triggerSchema;
  resultSchema;
  resultMapping;
  ...
  loop(applyOperator, condition, fallbackStep, loopType, variables) {
    ...
    const checkStep = {
      ...
      outputSchema: z3.z.object({
        status: z3.z.enum(["continue", "complete"])
      })
    };
    const checkStepNode = {
      step: checkStep,
...
//# sourceMappingURL=chunk-A4TGKXTE.cjs.map
//# sourceMappingURL=chunk-A4TGKXTE.cjs.map