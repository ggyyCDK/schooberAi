...
_Mastra_decorators = [chunkV6NA7QLK_cjs.InstrumentClass({
  prefix: "mastra",
...
exports.Mastra = class Mastra {
  ...
  generateId() {
    if (this.#idGenerator) {
      ...
      if (!id) {
        const error = new chunk3BFAQTC3_cjs.MastraError({
  ...
  constructor(config) {
    ...
    if (config?.logger === false) {
      ...
      if (config?.logger) {
        ...
        const levelOnEnv = process.env.NODE_ENV === "production" && process.env.MASTRA_DEV !== "true" ? chunkIDDUQR6P_cjs.LogLevel.WARN : chunkIDDUQR6P_cjs.LogLevel.INFO;
        logger = new chunkIDDUQR6P_cjs.ConsoleLogger({
          name: "Mastra",
          level: levelOnEnv
        });
      }
    ...
    this.#telemetry = chunkV6NA7QLK_cjs.Telemetry.init(config?.telemetry);
...
`
  ...
  setTelemetry(telemetry) {
    this.#telemetry = chunkV6NA7QLK_cjs.Telemetry.init(telemetry);
    if (this.#agents) {
      Object.keys(this.#agents).forEach(key => {
        if (this.#telemetry) {
...
//# sourceMappingURL=chunk-4L6RV2TV.cjs.map
//# sourceMappingURL=chunk-4L6RV2TV.cjs.map